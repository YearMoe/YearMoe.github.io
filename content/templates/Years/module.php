...
//个人信息
{
   ...
   <div class="widget YearMoe_post_author">
...
//微语
{
    ...
    <div class="widget widget_YearMoe_postlist">
...
if(isset($_POST['qq'])){
	if(empty($_POST['qq'])){
		echo "@@({comname:'QQ账号错误',commail:'QQ账号错误',comurl:'QQ账号错误',toux:'https://q.qlogo.cn/headimg_dl?bs=qq&dst_uin=admin@92mo.cn&src_uin=admin@92mo.cn&fid=blog&spec=100'})@@";
		return ;
	}
...
$img_url = TEMPLATE_URL.'images/cool.jpg';//设置没有图片时的显示

if(img_zw($row['content'])){$img_url = img_zw($row['content']);//调用文章内容的第一张图片
}elseif(img_zw($row['gid'])){$img_url = img_zw($row['gid']);//调用附件的第一张图片
}else{$img_url;}?>
<div class="single-item">
<div class="image" style="background-image:url(<?php echo $img_url;?>)">
    <a href="<?php echo Url::log($row['gid']);?>">
        <div class="overlay"></div>
        <div class="title"><span>随机推荐</span><h3><?php echo $row['title'];?></h3></div>
        </a>
	</div>
</div>
<?php }?>
...
//文章上下页
{
	...
	<?php if ($prevLog):?>
	<div class="prev-post">
	<a href="<?php echo Url::log($nextLog['gid']) ?>" title="<?php echo $nextLog['title']; ?>" class="prev has-background" style="background-image: url(<?php echo img_fj($prevLog['gid']); ?>)" alt="<?php echo $nextLog['title']; ?>">
