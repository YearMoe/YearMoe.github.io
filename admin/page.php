...
function get_files($dir) {
  ...
  if($handle) {
    while(false !== ($file = readdir($handle))) {
     if ($file != '.' && $file != '..') {
     $filename = $dir.'/'.$file;
     $tpls= str_replace($dir."/","",$filename);
     if(is_file($filename) && substr($tpls,0,4)=='page' && substr($filename,-4)=='.php') {
     $tplData = implode('', @file($dir.'/'.$file));
     preg_match("/Custom:(.*)/i", $tplData, $name);
     preg_match("/Description:(.*)/i", $tplData, $Des);
     $tplInfo['template'] = !empty($name[1]) ? trim($name[1]):'page';
     $tplInfo['Des'] = !empty($Des[1]) ? $Des[1] :'默认页面';
                    $files[] = $tplInfo;
                }else {
...
$dir = TPLS_PATH.Option::get('nonce_templet');
$tpls = get_files($dir);
//加载页面管理页面
if ($action == '') {
