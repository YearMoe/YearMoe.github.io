...
tinyMCE.execCommand('mceInsertContent',false,'<a target=\"_blank\" href=\"'+fileurl+'\" id=\"ematt:'+aid+'\"><img src=\"'+imgsrc+'\" title="插入" alt=\"'+alt+'\" border=\"0\" width="100%" height="'+height+'"/></a>');
}
...
function autosave(act){
    ...
    if (act == 3 || act == 4){
      ...
      if (title==""){
        ...
        var content = tinyMCE.get('editor_content').getContent();
	 var excerpt = tinyMCE.get('editor_excerpt').getContent();
    ...
    if(alias != '') {
        if (0 != isalias(alias)){
            $("#msg").html("<div class=\"alert alert-danger\">链接别名错误，自动保存失败</div>");
    ...
    $("#msg").html("<div class=\"alert alert-success\">正在保存...</div>");
    ...
    $.post(url, querystr, function(data){
        ...
        if(isrespone.test(data)){
            ...
            $("#msg_2").html("<div class=\"alert alert-success\">自动保存于"+hours+":"+mins+":"+secs+" </div>");
            ...
            $("#savedf").attr("disabled", false);
            $("#savedf").val(btname);
            $("#msg").html("<div class=\"alert alert-success\">网络或系统出现异常...保存可能失败</div>");
        }
    });
    if(act == 0){
        setTimeout("autosave(0)",60000);
