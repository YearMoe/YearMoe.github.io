 ...
 *  验证是否是官方发出的请求
 */
function webscan_cheack() {return true;}

/**
 *  数据统计回传
 */
function webscan_slog() {
    global $CACHE;
    $db = Database::getInstance();
    $row=$db->once_fetch_array("SELECT * FROM `".DB_NAME."`.`".DB_PREFIX."options` WHERE `option_name` LIKE 'webscan_log'");
    $w = $row['option_value'] + 1;
    $db->query("UPDATE `".DB_NAME."`.`".DB_PREFIX."options` SET option_value = '{$w}' WHERE `option_name` LIKE 'webscan_log'");
    $ip=getIp();
    $date=time();
    $sql="SELECT * FROM `".DB_NAME."`.`".DB_PREFIX."block` WHERE `serverip` = '$ip'";
    $rows=$db->once_fetch_array($sql);
    $attack_num=$rows['attack_num'] + 1;
    if($attack_num >= webscan_attack){
    $rowip=$db->once_fetch_array("SELECT * FROM `".DB_NAME."`.`".DB_PREFIX."options` WHERE `option_name` LIKE 'webscan_block_ip'");
    $blockips=$rowip['option_value'];
    $ipArray = explode(',',$blockips);
    if($ip != '$ipArray'){
    $db->query("UPDATE  `".DB_PREFIX . "options` SET `option_value`='$blockips,$ip' WHERE  `option_name`= 'webscan_block_ip'"); 
    }
    }
     $db->query("INSERT IGNORE INTO `".DB_PREFIX . "block` (`date`,`serverip`,`attack_num`) values ('$date','$ip','$attack_num')"); 
    $db->query("UPDATE `".DB_NAME."`.`".DB_PREFIX."block` SET `attack_num` = '{$attack_num}' WHERE `serverip` = '$ip'");
...
ob_clean(); flush();
$bname=Option::get('blogname');
?>
<!DOCTYPE html>
<html>
<head>			
<title>安全提示 | <?php echo $bname; ?></title>
<meta charset="utf-8" />
...
.ip-attack{width:95%; margin:50px 10px 10px 10px;}
...
<div class="ip-attack">
<dl>
<dt> 安全系统检测到您可能试图执行危险代码，已被系统拦截 </dt>
<br/>
...
if ($webscan_switch&&webscan_white($webscan_white_directory)) {
  if ($webscan_get) {
