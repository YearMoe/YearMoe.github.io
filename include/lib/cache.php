...
class Cache {
    ...
    private function mc_sta() {
        ...
        $data = $this->db->once_fetch_array("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "link ");
		$linknum = $data['total'];
...
$link_cache = array();
		$query = $this->db->query("SELECT siteurl,sitename,sitepic,linksortid,description FROM " . DB_PREFIX . "link WHERE hide='n' ORDER BY taxis ASC");
		while ($show_link = $this->db->fetch_array($query)) {
			$link_cache[] = array(
				'link' => htmlspecialchars($show_link['sitename']),
				'url' => htmlspecialchars($show_link['siteurl']),
				'sitepic' => htmlspecialchars($show_link['sitepic']),
				'linksortid' => htmlspecialchars($show_link['linksortid']),
	 ...
	 * 友情链接分类缓存
	 */
	private function mc_sortlink() {
		$sortlink_cache = array();
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "sortlink ORDER BY taxis ASC");
		while ($row = $this->db->fetch_array($query)) {
			$data = $this->db->once_fetch_array("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "link WHERE linksortid=" . $row['linksort_id'] . " ");
			$linkNum = $data['total'];
			$sortlinkData = array(
				'linknum' => $linkNum,
				'linksort_name' => htmlspecialchars($row['linksort_name']),
				'linksort_id' => intval($row['linksort_id']),
				'taxis' => intval($row['taxis']),
				);
			$sortlink_cache[$row['linksort_id']] = $sortlinkData;
		}
		$cacheData = serialize($sortlink_cache);
		$this->cacheWrite($cacheData, 'sortlink');
	}
    /**
     * 导航缓存
     */
