...
function blockIP_check()
{
        $flag = false;
        $ip = getIp();
        $iptable = getAllBlockIP();
        if ($iptable) {
            foreach ($iptable as $value) {
                if (preg_match("{$value}", $ip)) {
                    $flag = true;
                    emMsg("抱歉,你是不是对本站做了啥,才被关进了小黑屋,过段时间再试吧"); 
                    break;
                }
            }
        }
...
function emLoadJQuery() {
    ...
    if (!$isJQueryLoaded) {
        ...
        function loadJQuery() {
            echo '<script src="' . BLOG_URL . 'include/lib/js/jquery/jquery-1.11.0.js" type="text/javascript"></script>';
        }
...
function emoFormat($t){
    $emos = array('[微笑]'=>'0.gif', '[撇嘴]'=>'1.gif', '[色]'=>'2.gif', '[发呆]'=>'3.gif', '[得意]'=>'4.gif', '[流泪]'=>'5.gif', '[害羞]'=>'6.gif', '[发怒]'=>'7.gif', '[憨笑]'=>'8.gif', '[调皮]'=>'9.gif', '[擦汗]'=>'10.gif', '[惊恐]'=>'11.gif', '[疑问]'=>'12.gif', '[晕]'=>'13.gif', '[抓狂]'=>'14.gif', '[坏笑]'=>'15.gif', '[阴险]'=>'16.gif', '[亲亲]'=>'17.gif', '[吓]'=>'18.gif', '[可怜]'=>'19.gif', '[傲慢]'=>'20.gif', '[龇牙]'=>'21.gif');
    if(!empty($t) && preg_match_all('/\[.+?\]/',$t,$matches)){
        $matches = array_unique($matches[0]);
        foreach ($matches as $data) {
            if(isset($emos[$data]))
                $t = str_replace($data,'<img class="tsmile" title="'.$data.'" src="'.BLOG_URL.'admin/tinymce/tinymce/plugins/emoticons/img/'.$emos[$data].'"/>',$t);
        }
    }
    return $t;
}
